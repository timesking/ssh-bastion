...
func (l *LogChannel) SyncToFile(remote_name string) error {
	...
	startStr := strings.Replace(l.StartTime.Format(time.RFC3339Nano), ":", "-", -1)
	filename := filepath + "/" + fmt.Sprintf("ssh_log_%s_%s_%s", startStr, l.UserName, remote_name)
	l.filename = filename
	log.Printf("Logging Start %s.", filename)
	l.logMutex.Lock()
...
func (l *LogChannel) Close() error {
	...
	log.Printf("Logging Stop %s.", l.filename)
